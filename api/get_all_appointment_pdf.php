 ...
 $array = implode("','",$array);
    ...
    if(($doctId=="")){
      ...
      $result=mysqli_query($conn,"select * from appointments where  `appointmentStatus` IN ('".$array."') AND date_c >='$firstDate' AND date_c <= '$lastDate' ORDER BY STR_TO_DATE(appointmentDate,'%m-%d-%Y') DESC, appointmentTime DESC");//LIMIT $limit
             ...
             if($firstDate=="All"&&$lastDate=="All" ){
       $result=mysqli_query($conn,"select * from appointments where `doctId`='$doctId' AND `appointmentStatus` IN ('".$array."') 
      ORDER BY STR_TO_DATE(appointmentDate,'%m-%d-%Y') DESC, appointmentTime DESC");
}
else{
      $result=mysqli_query($conn,"select * from appointments where `doctId`='$doctId' AND `appointmentStatus` IN ('".$array."') AND date_c >='$firstDate' AND date_c <= '$lastDate' ORDER BY STR_TO_DATE(appointmentDate,'%m-%d-%Y') DESC, appointmentTime DESC");//LIMIT $limit
}
