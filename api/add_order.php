...
include('send_user_mail.php');
    ...
    if($insertPayments){
        ...
        //   send emails
        $orderDate = date("jS F, Y");
        $region = $state . ", " . $coutry;
        sendAdminNotification('sales@epigensol.co.uk', $first_name, $last_name, $email, $phone_number, $last_id_p, $orderDate, $shipping_address, $zip, $region, $paidAmt);
        sendPurchaseConfirmation($email, $first_name, $last_id_p, $orderDate, $paidAmt);
            
    if($insert){
        $last_id = mysqli_insert_id($conn);
  
        $result=$conn->query("SELECT cart.*,  product.price, product.tax FROM cart INNER JOIN product ON product.id=cart.product_id where cart.uid='$uid'");
        $data=array();
